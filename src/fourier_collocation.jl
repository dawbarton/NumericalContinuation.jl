...
struct FourierCollocation{F, T}
    ...
    phase::Bool
    Dt::Matrix{T}
    du::Vector{T}  # TODO: think about how to put this into chart data to avoid problems with recomputation
    function FourierCollocation(f, u, tspan, p = (); eqns = missing, phase::Bool = true)
        # Boilerplate
        ...
        T = eltype(u)
        # Specific to Fourier collocation
        nmesh = size(u, 2)
        Dt = fourier_diff(T, nmesh) .* -2π  # Scale to [0, 1] and transpose (negative = transpose)
        _vars = nmesh * ndim + length(p) + 2
        _eqns = nmesh * ndim + phase
        du = vec((u * Dt) .* (1 / nmesh))  # time derivative of initial solution scaled by the number of mesh points
        return new{typeof(_f), T}(_f,
                                  (u = vec(u), p = p, tspan = [tspan[begin], tspan[end]]),
                                  _vars, _eqns, ndim, nmesh, phase, Dt, du)
    end
end
...
function (fourier::FourierCollocation{F, TT})(res, u, data; kwargs...) where {F, TT}
    ...
    res_mat = reshape(@view(res[1:(fourier.ndim * fourier.nmesh)]),
                      (fourier.ndim, fourier.nmesh))
    mul!(res_mat, u_mat, fourier.Dt, -one(TT), one(TT))
    if fourier.phase
        res[end] = dot(fourier.du, u.u)
    end
    return res
end
